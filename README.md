# Ice Cream Nutrition Tracker (Backend)

Минимальный проект для хакатона: бекенд + Postgres, Docker, CRUD, auth, расчёт калорий/БЖУ/сахара по съеденному мороженному.
...
API будет доступен на `http://localhost:3000`, фронт откроется по корню `/`, документация FastAPI на `http://localhost:3000/docs`.
...
- `static/index.html` — минимальный фронтенд.
